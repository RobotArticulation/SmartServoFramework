 ...
 *
 * \file ServoMercury.cpp
 * \date 13/03/2014
 * \author Emeric Grange <emeric.grange@gmail.com>
 */

#include "mercury/ServoMercury.h"
...
ServoMercury::ServoMercury(const int control_table[][8], int mercury_id, int mercury_model, int speed_mode):
    Servo()
{
    // Store servo id, serie and model
    servoId = mercury_id;
    mcy_get_model_infos(mercury_model, servoSerie, servoModel);
    ...
    registerTableValues[gid(REG_MODEL_NUMBER)] = mercury_model;
    if (mercury_id > -1 && mercury_id < BROADCAST_ID)
    {
        registerTableValues[gid(REG_ID)] = mercury_id;
    }
    ...
    // Set default value for ack policy, ACK_REPLY_ALL for mercury devices
...
void ServoMercury::status()
{
    ...
    TRACE_INFO(DXL, "> baudrate   : %i", mcy_get_baudrate(registerTableValues[gid(REG_BAUD_RATE)]));
...
std::string ServoMercury::getModelString()
{
    std::lock_guard <std::mutex> lock(access);
    return mcy_get_model_name(registerTableValues[gid(REG_MODEL_NUMBER)]);
}

void ServoMercury::getModelInfos(int &servo_serie, int &servo_model)
{
    std::lock_guard <std::mutex> lock(access); // ?
...
int ServoMercury::getSpeedMode()
{
    return speedMode;
}

void ServoMercury::setSpeedMode(int speed_mode)
{
    if (speed_mode == SPEED_MANUAL || speed_mode == SPEED_AUTO)
    {
        speedMode = speed_mode;
...
void ServoMercury::waitMovementCompletion(int timeout_ms)
{
...
int ServoMercury::getBaudRate()
{
    std::lock_guard <std::mutex> lock(access);
    return mcy_get_baudrate(registerTableValues[gid(REG_BAUD_RATE)], servoSerie);
}

int ServoMercury::getReturnDelay()
{
    std::lock_guard <std::mutex> lock(access);
    return registerTableValues[gid(REG_RETURN_DELAY_TIME)];
}

double ServoMercury::getHighestLimitTemp()
{
    std::lock_guard <std::mutex> lock(access);
    return static_cast<double>(registerTableValues[gid(REG_TEMPERATURE_LIMIT)]);
}

double ServoMercury::getLowestLimitVolt()
{
    std::lock_guard <std::mutex> lock(access);
...
double ServoMercury::getHighestLimitVolt()
{
    std::lock_guard <std::mutex> lock(access);
...
void ServoMercury::setId(int id)
{
...
void ServoMercury::setCWLimit(int limit)
{
...
void ServoMercury::setCCWLimit(int limit)
{
...
void ServoMercury::setGoalPosition(int pos)
{
...
void ServoMercury::setGoalPosition(int pos, int time_budget_ms)
{
...
void ServoMercury::moveGoalPosition(int move)
{
...
void ServoMercury::setMovingSpeed(int speed)
{
...
void ServoMercury::setMaxTorque(int torque)
{
...
void ServoMercury::setLed(int led)
{
...
void ServoMercury::setTorqueEnabled(int torque)
{
