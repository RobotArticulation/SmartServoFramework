#ifndef MERCURY_SIMPLE_API_H
#define MERCURY_SIMPLE_API_H
...
/*!
 * \brief The MercurySimpleAPI class
 *
 ...
 * - 'SERVO_MX' should be fine for almost every use cases when using Mercury v1 devices.
 * - 'SERVO_DRS' should be fine for almost every use cases when using Mercury v1 devices.
 *
 ...
 */
class MercurySimpleAPI: public Mercury
{
...
public:
    /*!
     * \brief MercurySimpleAPI constructor.
     * \param servoSerie: The servo serie to use with this instance. Default is Mercury MX, the more 'capable' of the Mercury v1 series.
     */
    MercurySimpleAPI(int servoSerie = SERVO_MX);

    /*!
     * \brief MercurySimpleAPI destructor.
     *
     ...
     * \return A vector of Mercury IDs found during the scan.
     *
     ...
     *
     * Only Mercury "v2" devices can be rebooted (X series, PRO series).
     ...
     *
     * Be careful when resetting a Mercury 'v1' device, its ID will be reseted to 1.
     */
